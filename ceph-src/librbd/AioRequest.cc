...
namespace librbd {
  ...
  AioRequest::AioRequest(ImageCtx *ictx, int pool, const std::string &oid,
			 ...
			 Context *completion,
			 bool hide_enoent) :
    m_ictx(ictx), m_ioctx(&ictx->data_ctx[pool]), m_oid(oid), m_object_no(objectno),
  ...
  AbstractWrite::AbstractWrite()
    : m_state(LIBRBD_AIO_WRITE_FLAT),
      m_parent_overlap(0) {}
  AbstractWrite::AbstractWrite(ImageCtx *ictx, int pool, const std::string &oid,
			       ...
			       Context *completion,
			       bool hide_enoent)
    : AioRequest(ictx, pool, oid, object_no, object_off, len, snap_id, completion,
		 hide_enoent),
  ...
  void AbstractWrite::send_copyup() {
    ...
    librados::AioCompletion *rados_completion =
      librados::Rados::aio_create_completion(this, NULL, rados_req_cb);
    m_ictx->md_ctx[pool].aio_operate(m_oid, rados_completion, &m_copyup,
			       m_snap_seq, m_snaps);
    rados_completion->release();
  }
}
