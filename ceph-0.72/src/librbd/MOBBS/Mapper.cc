...
Mapper::Mapper(string object_name):m_cluster_name("ceph"),m_user_name("client.admin"),
	m_ceph_conf("/etc/ceph/ceph.conf"), m_pool_name("hdd-pool"),m_object_name(object_name)
{
	load_extent_map();
...
void Mapper::save_extent_map()
{
	...
	*/
	...
	ret = io_ctx.write_full(m_object_name, bl);
	if(ret < 0)
	{
