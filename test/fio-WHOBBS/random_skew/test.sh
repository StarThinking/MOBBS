...
function test() {
  ...
  if [ $zipf = 0 ]
  then
    ssh 192.168.122.53 "fio -filename=/dev/vdb -direct=1  -rw=randwrite -bs=$size -size=$disk_size -numjobs=32 -runtime=$runtime -group_reporting --name=test; sleep $sleeptime;"
  else
    ssh 192.168.122.53 "fio -filename=/dev/vdb -direct=1  -rw=randwrite -random_distribution=zipf:$zipf -bs=$size -size=$disk_size -numjobs=32 -runtime=$runtime -group_reporting --name=test; sleep $sleeptime;"
  fi
  after $testname
...
#test 1.75 6000 100 zipf-1.75
test 1.5 8000 100 zipf-1.5
test 1.25 9000 100 zipf-1.25
test 0 10000 100 zipf-0
