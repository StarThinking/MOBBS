...
namespace librbd {
  ...
  public:
    ...
    virtual void finish(int r) {
      cout << "LibrbdWriteback.cc: finish"<< std::endl;
			     ...
			     __u32 trunc_seq, Context *onfinish)
  {
    ...
    cout << "LibrbdWriteback.cc read" << std::endl;
    ...
    int r = m_ictx->data_ctx[pool].aio_operate(oid.name, rados_completion, &op,
					 snapid, flags, NULL);
  ...
  {
    cout << "LibrbdWriteback.cc may_copy_on_write" << std::endl;
  ...
  tid_t LibrbdWriteback::write(const object_t& oid,
			       ...
			       Context *oncommit)
  {
    cout << "LibrbdWriteback.cc write" << std::endl;
    int pool = 0;
    m_ictx->snap_lock.get_read();
    librados::snap_t snap_id = m_ictx->snap_id;
    m_ictx->parent_lock.get_read();
    ...
    AioWrite *req = new AioWrite(m_ictx, pool, oid.name,
  ...
  {
    cout << "LibrbdWriteback.cc complete_writes" << std::endl;
