...
namespace librbd {
  ...
  {
    int pool = 0;
    CephContext *cct = (CephContext *)ictx->data_ctx[pool].cct();
	...
	ictx->data_ctx[pool].aio_remove(oid, rados_completion);
	...
	librados::AioCompletion *rados_completion =
	  librados::Rados::aio_create_completion(req_comp, NULL, rados_ctx_cb);
	if (p->offset == 0) {
	  ictx->data_ctx[pool].aio_remove(p->oid.name, rados_completion);
	} else {
	  librados::ObjectWriteOperation op;
	  op.truncate(p->offset);
	  ictx->data_ctx[pool].aio_operate(p->oid.name, rados_completion, &op);
	}
	rados_completion->release();
      }
    }
...
#define READ_SIZE 4096
		     ...
		     ProgressContext& prog_ctx)
  {
	...
	librados::Rados::aio_create_completion(req_comp, NULL, rados_ctx_cb);
      ...
      ictx->data_ctx[pool].aio_operate(oid, rados_completion, &op);
  ...
  int list_children(ImageCtx *ictx, set<pair<string, string> >& names)
  {
    ...
    parent_spec parent_spec(ictx->md_ctx[pool].get_id(), ictx->id, ictx->snap_id);
    names.clear();

    // search all pools for children depending on this snapshot
    Rados rados(ictx->md_ctx[pool]);
  ...
  int snap_create(ImageCtx *ictx, const char *snap_name)
  {
    ...
    notify_change(ictx->md_ctx[pool], ictx->header_oid, NULL, ictx);
  ...
  int snap_remove(ImageCtx *ictx, const char *snap_name)
  {
	...
	return r;
	  ...
	  r = cls_client::remove_child(&ictx->md_ctx[pool], RBD_CHILDREN,
				       our_pspec, ictx->id);
	  if (r < 0)
	    return r;
      }
    ...
    r = ictx->data_ctx[pool].selfmanaged_snap_remove(snap_id);
  ...
  int snap_protect(ImageCtx *ictx, const char *snap_name)
  {
    ...
    r = cls_client::set_protection_status(&ictx->md_ctx[pool],
					  ictx->header_oid,
					  snap_id,
  ...
  int snap_unprotect(ImageCtx *ictx, const char *snap_name)
  {
    ...
    parent_spec pspec(ictx->md_ctx[pool].get_id(), ictx->id, snap_id);
    // search all pools for children depending on this snapshot
    Rados rados(ictx->md_ctx[pool]);
...
reprotect_and_return_err:
    int proterr = cls_client::set_protection_status(&ictx->md_ctx[pool],
						    ictx->header_oid,
						    snap_id,
					      RBD_PROTECTION_STATUS_PROTECTED);
    if (proterr < 0) {
	    ...
	    uint64_t stripe_unit, int stripe_count)
  {
    ...
    c_imctx = new ImageCtx(c_name, "", NULL, c_ioctx, c_ioctx, false);
  ...
  int open_parent(ImageCtx *ictx)
  {
    ...
    ictx->parent = new ImageCtx("", parent_image_id, NULL, p_ioctx, p_ioctx, true);
		      ...
		      string *parent_name, string *parent_snap_name)
  {
    ...
    if (parent_name) {
      r = cls_client::dir_get_name(&ictx->parent->md_ctx[pool], RBD_DIRECTORY,
  ...
  int remove(IoCtx& io_ctx, const char *imgname, ProgressContext& prog_ctx)
  {
    ...
    ImageCtx *ictx = new ImageCtx(imgname, "", NULL, io_ctx, io_ctx, false);
  ...
  int resize_helper(ImageCtx *ictx, uint64_t size, ProgressContext& prog_ctx)
  {
    ...
    if (ictx->old_format) {
      ...
      r = ictx->md_ctx[pool].write(ictx->header_oid, bl, bl.length(), 0);
    } else {
      r = cls_client::set_size(&(ictx->md_ctx[pool]), ictx->header_oid, size);
    }
  ...
  int add_snap(ImageCtx *ictx, const char *snap_name)
  {
    ...
    int r = ictx->md_ctx[pool].selfmanaged_snap_create(&snap_id);
    if (r < 0) {
    ...
    if (ictx->old_format) {
      r = cls_client::old_snapshot_add(&ictx->md_ctx[pool], ictx->header_oid,
				       snap_id, snap_name);
    } else {
      r = cls_client::snapshot_add(&ictx->md_ctx[pool], ictx->header_oid,
				   snap_id, snap_name);
    }
  ...
  int rm_snap(ImageCtx *ictx, const char *snap_name)
  {
    ...
    int r;
    if (ictx->old_format) {
      r = cls_client::old_snapshot_remove(&ictx->md_ctx[pool],
					  ictx->header_oid, snap_name);
    } else {
      RWLock::RLocker l(ictx->snap_lock);
      r = cls_client::snapshot_remove(&ictx->md_ctx[pool],
				      ictx->header_oid,
  ...
  int refresh_parent(ImageCtx *ictx) {
	...
	return r;
      if (!overlap ||
	  ictx->parent->md_ctx[pool].get_id() !=
  ...
  int ictx_refresh(ImageCtx *ictx)
  {
	...
	ictx->lockers.clear();
	if (ictx->old_format) {
	  r = read_header(ictx->md_ctx[pool], ictx->header_oid, &ictx->header, NULL);
	  if (r < 0) {
	    lderr(cct) << "Error reading header: " << cpp_strerror(r) << dendl;
	    return r;
	  }
	  r = cls_client::old_snapshot_list(&ictx->md_ctx[pool], ictx->header_oid,
	  ...
	  r = rados::cls::lock::get_lock_info(&ictx->md_ctx[pool], ictx->header_oid,
	  ...
	  do {
	    ...
	    r = cls_client::get_mutable_metadata(&ictx->md_ctx[pool], ictx->header_oid,
	    ...
	    r = cls_client::snapshot_list(&(ictx->md_ctx[pool]), ictx->header_oid,
	...
	ictx->snap_exists = false;
      ...
      ictx->data_ctx[pool].selfmanaged_snap_set_write_ctx(ictx->snapc.seq, ictx->snaps);
    } // release snap_lock
  ...
  int flatten(ImageCtx *ictx, ProgressContext &prog_ctx)
  {
	...
	if (!ictx->parent) {
      ...
      AioWrite *req = new AioWrite(ictx, pool, oid, ono, 0, objectx, object_overlap,
    ...
    r = cls_client::remove_parent(&ictx->md_ctx[pool], ictx->header_oid);
    if (r < 0) {
    ...
    if (ictx->snaps.empty()) {
      ...
      int r = cls_client::remove_child(&ictx->md_ctx[pool], RBD_CHILDREN,
	   ...
	   const string& tag)
  {
     ...
     */
    RWLock::RLocker locker(ictx->md_lock);
    r = rados::cls::lock::lock(&ictx->md_ctx[pool], ictx->header_oid, RBD_LOCK_NAME,
  ...
  int unlock(ImageCtx *ictx, const string& cookie)
  {
    ...
    RWLock::RLocker locker(ictx->md_lock);
    r = rados::cls::lock::unlock(&ictx->md_ctx[pool], ictx->header_oid,
		 ...
		 const string& cookie)
  {
    ...
    RWLock::RLocker locker(ictx->md_lock);
    r = rados::cls::lock::break_lock(&ictx->md_ctx[pool], ictx->header_oid,
		   ...
		   void *arg)
  {
    ...
    ictx->md_lock.get_read();
    ictx->snap_lock.get_read();
    head_ctx.dup(ictx->data_ctx[pool]);
  ...
  int aio_flush(ImageCtx *ictx, AioCompletion *c)
  {
    ...
    if (ictx->object_cacher) {
      ...
      librados::AioCompletion *rados_completion =
	librados::Rados::aio_create_completion(req_comp, NULL, rados_ctx_cb);
      ictx->data_ctx[pool].aio_flush_async(rados_completion);
      rados_completion->release();
    }
  ...
  int _flush(ImageCtx *ictx)
  {
    ...
    if (ictx->object_cacher) {
      ...
      r = ictx->data_ctx[pool].aio_flush();
    }
  ...
  int aio_write(ImageCtx *ictx, uint64_t off, size_t len, const char *buf,
		AioCompletion *c)
  {
    //printf("aio_write, off = %ld, len = %ld\n", off, len);
   ...
   // cout << "pid: " << tid << " internal.cc: aio_write " << ictx << " off = " << off << " len = "
		   //<< len << " buf = " << &buf << std::endl;
  ...
  int aio_discard(ImageCtx *ictx, uint64_t off, uint64_t len, AioCompletion *c)
  {
	...
	req = new AioRemove(ictx, pool, p->oid.name, p->objectno, objectx, object_overlap,
			    snapc, snap_id, req_comp);
      } else if (p->offset + p->length == ictx->layout.fl_object_size) {
	req = new AioTruncate(ictx, pool, p->oid.name, p->objectno, p->offset, objectx, object_overlap,
			      snapc, snap_id, req_comp);
      } else {
	req = new AioZero(ictx, pool, p->oid.name, p->objectno, p->offset, p->length,
			  objectx, object_overlap,
			  snapc, snap_id, req_comp);
      }
	       ...
	       char *buf, bufferlist *pbl, AioCompletion *c, int pool)
  {
	...
	if (ictx->object_cacher) {
	  cout << "internal.cc: object_cacher" << std::endl;
