...
def parseFile(desFile):
	writer = csv.writer(desFile)

	writer.writerow(['####', '####', '4k workload', '####', '####'])
	writer.writerow([' '] + [' ','RAND_READ',' ',' ','SEQ_READ',' '] + [' ', 'RAND_WRITE',' ',' ','SEQ_WRITE',' '])
	writer.writerow(['SIZE'] + ['IOPS','BW_KB','BW_MB'] * 4)

	r1 = readFile('ran-read')
	r2 = readFile('seq-read')
	r3 = readFile('ran-write')
	r4 = readFile('seq-write')
